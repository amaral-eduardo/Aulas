...
### Plotando S(t) estimada
dados_sob <- Surv(tempo0$time, tempo0$status) # Convertendo para formato de dados de sobrev.


km1 <- survfit(dados_sob~1, conf.type='plain')  # Plain é sem ajuste
# Plain tbm limita no intervalo de 0 (lim inferior) a 1 (lim superior) a prob.

km2 <- survfit(dados_sob~1, conf.type='log-log') # Correção log-log
...
surv <- km1$surv #Prob de sobrev estimada
time <- km1$time #Tempos de falha
